...
image = Image.open('data/diabetes3.png')
...
if (st.button('Find Health Status')):
    ...
    if result != "This is a healthy person!":
        color = 'Red'
        result_image = Image.open('data/diabetes1.jpg') # unhealth.jpg
    else:
        color = 'Green'
        result_image = Image.open('data/health.jpeg') # health3.jpg
    main_image.empty()
    # main_info.empty()
    result_message = '<p style="font-size:30px; text-align: center; color:'+color+';">'+result+'</p>'
